...
source('global.R')
data.directory = '~/MEGA/data_files'
```
...
read.counts <- importStarCounts(folder = file.path(data.directory, "/star_counts/"), 
                                column = 4)
...
write.table(x = read.counts,
            file = file.path(data.directory, "RNAseq_read_counts", "all_read.counts.txt"),
            sep = "\t",
            col.names = T)
...
write.table(x = read.counts, 
            file = file.path(data.directory, "RNAseq_read_counts", "GSC_and_nonGSC_readcounts.txt"),
            sep = "\t",
            col.names = T)
...
voom.counts <- voom(counts = dge.counts, 
                    design = design,
                    plot=TRUE)
#MDS plot of the data
plotMDS(x = dge.counts, 
        cex = 1,
...
voom.all_HGNC <- EnsembltoHGNC(voom.all)
...
write.table(x = voom.all_HGNC, 
            file = file.path(data.directory, "RNAseq_expressed_genes", "GSC_vs_nonGSC_all_differentially_expressed_genes.csv"),
            quote = FALSE, 
            sep = ",", 
            row.names = T)
write.table(x = voom_top_HGNC,
            file = file.path(data.directory, "RNAseq_expressed_genes", "GSC_vs_nonGSC_top_pval05_differentially_expressed_genes.csv"), 
            quote = FALSE, 
            sep = ",",
            row.names = T)
#write.table(x = as.data.frame(voom.counts$E), "../data/star_v5_normalized_counts.txt", quote = FALSE,  col.names = TRUE, sep = ",")
...
conversion <- read.table(file = file.path(data.directory, "conversion_sets", "ensembl_to_entrez.txt"), 
                         stringsAsFactors = F, 
                         sep = "\t", 
                         header = T)
stats <- as.data.frame(voom.all_HGNC$logFC)
stats$hgnc_symbol <- voom.all_HGNC$hgnc_symbol
#stats <- stats[stats$symbol != "",]
stats <- merge(stats,conversion, by = 'hgnc_symbol')
ranks <- stats$`voom.all_HGNC$logFC`
...
pdf(file.path(data.directory, "GSEA", "GSC_vs_nonGSC_gsea.pdf", 14,8))
...
```{r GSC_vs_HNPC}
# load in the star counts
read.counts <-importStarCounts(file.path(data.directory, "star_counts/"), column = 4)
# removing uneeded text from column names
colnames(read.counts) <- gsub(pattern = "_dna_ReadsPerGene.out.tab", 
                              replacement = "",
                              colnames(read.counts))
...
# write the read counts for later reference
write.table(x = read.counts,
            file = file.path(data.directory, "GSEA", "GSC_vs_HNPC_read.counts.txt"),
            sep = "\t",
            col.names = T)
...
read.counts <- 
  read.counts %>%
  subset(select = -c(`387-`,`456-`, `3359-`,`3691-`,`3832-`,`4121-`,`4302-`,`H2S-`,
...
# reads with low counts be removed
...
#voom fold changes and p values for all genes
voom.all_genes <- topTable(fit = fit, 
                           coef = "celltypeGSC",
                           sort.by="p",
                           number =nrow(voom.counts))
#voom fold changes and p values based on significant hits
...
#name conversion to HGNC symbols
voom.all_genes_HGNC <- EnsembltoHGNC(voom.all_genes)
voom_top_HGNC <- EnsembltoHGNC(voom_top)
...
write.table(x = voom.all_HGNC, 
            file = file.path(data.directory, "RNAseq_expressed_genes", "GSC_vs_HNPC_all_differentially_expressed_genes.csv"),
            quote = FALSE,
            sep = ",",
            row.names = F)
write.table(x = voom_top_HGNC, 
            file = file.path(data.directory, "RNAseq_expressed_genes", "GSC_vs_HNPC_top_pval05_differentially_expressed_genes.csv"),
            quote = FALSE, 
            sep = ",",
            row.names = F)
...
pdf(file.path(data.directory, "GSEA", "GSC_vs_HNPC_gsea.pdf"), 14,8)
topPathwaysUp <- fgseaRes[ES > 0][head(order(pval), n=10), pathway]
topPathwaysDown <- fgseaRes[ES < 0][head(order(pval), n=10), pathway]
topPathways <- c(topPathwaysUp, rev(topPathwaysDown))
print(
...
read.counts <- read.table(file.path(data.directory, "RSEM_out", "rsem_isoforms_matrix.txt"))
...
conversion <- read.table(file = file.path(data.directory, "conversion_sets", "ensembl_conversion.txt"),
                         stringsAsFactors = F, 
                         sep = "\t",
                         header = T)
...
voom.all$transcript_id <- rownames(voom.all)
voom.all_HGNC <- merge(x = voom.all,
                       y = conversion[,c(2,3)], 
                       by = "transcript_id")

# write the differently expressed transcripts to file
write.table(x = voom.all_HGNC,
            file = file.path(data.directory, "RNAseq_expressed_genes", "GSC_vs_nonGSC_all_differentially_expressed_transcripts.csv"),
            quote = FALSE, 
            sep = ",",
            row.names = F)
write.table(x = voom_top_HGNC, 
            file = file.path(data.directory, "RNAseq_expressed_genes", "GSC_vs_nonGSC_top_pval05_differentially_expressed_transcripts.csv"),
            quote = FALSE, 
            sep = ",",
            row.names = F)
...
#load in the star counts
read.counts <- importStarCounts(folder = file.path(data.directory, "SRP040525_gene_counts/"), 
                                column = 4)
...
#write.table(x = voom.all_HGNC, "../data/StemVsDiff_all_DE_genes.txt", quote = FALSE, sep = ",", row.names = F)
